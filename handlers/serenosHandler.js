const {
  ...
  getCargoSereno,
  getJurisdicciones,
} = require("../controllers/serenosController");
...
const getAllSerenosHandler = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error interno del servidor al obtener los serenos" });
  }
};
...
const getSerenoByDNIHandler = async (req, res) => {
  ...
  try {
    ...
    if (!sereno) {
      return res
        .status(200)
        .json({ message: "Sereno no encontrado", data: {} });
    }

    const { id, nombres, apellidoPaterno, apellidoMaterno, cargo_sereno_id } =
      sereno.data[0];
    ...
    res
      .status(500)
      .json({ error: "Error interno del servidor al obtener el sereno" });
  }
};
...
const getCargoSerenoHandler = async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "Error interno del servidor al obtener los cargos de sereno",
    });
  }
};
...
const getJurisdiccionesHandler = async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "Error interno del servidor al obtener las juriscicciones",
    });
  }
};
